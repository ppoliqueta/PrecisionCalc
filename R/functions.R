# =======================================================
# PrecisionCalc R functions - all in one file
# =======================================================
#' PrecisionCalc: Sample Size and Distribution Analysis
#'
#' The PrecisionCalc package provides tools to compute required sample sizes
...
plot_distributionsC <- function(y)
{
  ...
  fit_t <- tryCatch({
    fitdist(y, "t", start = list(df = length(y) - 1), method = "mle")
  }, error = function(e) {
  ...
  fit_gamma <- fitdist(y_pos, "gamma", method = "mle")
  shape_gamma <- fit_gamma$estimate["shape"]
  rate_gamma <- fit_gamma$estimate["rate"]

  fit_lognorm <- fitdist(y_pos, "lnorm", method = "mle")
...
#' @export
relative_sample <- function(y, epsilon, conf = 0.95, Discrete = TRUE, N = NULL) {
  # Load required packages (outside a loop is better in practice)
  if (!requireNamespace("MASS", quietly = TRUE)) stop("Package 'MASS' is required.")
  if (!requireNamespace("fitdistrplus", quietly = TRUE)) stop("Package 'fitdistrplus' is required.")
  ...
  # Normal distribution
  sample_size_normal_rel <- function(mu, sigma, epsilon, conf = 0.95, N = NULL) {
    z <- qnorm(1 - (1 - conf)/2)
    n0 <- (z^2 * sigma^2) / ((epsilon * mu)^2)
    if (!is.null(N)) {
      n_adj <- n0 / (1 + (n0 - 1) / N)
  ...
  sample_size_P_rel <- function(lambda, epsilon, N = NULL) {
    n0 <- 1 / (lambda * epsilon^2)
    if (!is.null(N)) {
      n_adj <- n0 / (1 + (n0 - 1) / N)
      return(list(n0 = ceiling(n0), n_adj = ceiling(n_adj)))
    } else {
  ...
  sample_size_NB_rel <- function(mu, theta, epsilon, conf = 0.95, N = NULL) {
    z <- qnorm(1 - (1 - conf)/2)
    var_Y <- mu + (mu^2 / theta)
    n0 <- (z^2 * var_Y) / ((epsilon * mu)^2)
    if (!is.null(N)) {
      n_adj <- n0 / (1 + (n0 - 1) / N)
      return(list(n0 = ceiling(n0), n_adj = ceiling(n_adj)))
    } else {
  ...
  # Gamma distribution
  sample_size_Gamma_rel <- function(alpha, beta, epsilon, conf = 0.95, N = NULL) {
    z <- qnorm(1 - (1 - conf)/2)
    mu <- alpha / beta
    var_Y <- alpha / (beta^2)
    n0 <- (z^2 * var_Y) / ((epsilon * mu)^2)
  ...
  sample_size_Lognorm_rel <- function(meanlog, sdlog, epsilon, conf = 0.95, N = NULL) {
    z <- qnorm(1 - (1 - conf)/2)
  ...
  sample_size_t_rel <- function(x, epsilon, conf = 0.95, N = NULL) {
  ...
  if (Discrete) {
    results$Normal   <- sample_size_normal_rel(mu, sigma, epsilon, conf, N)
    results$t        <- sample_size_t_rel(y, epsilon, conf, N)
    results$Poisson  <- sample_size_P_rel(lambda, epsilon, N)
    results$NegBinom <- sample_size_NB_rel(mu, theta, epsilon, conf, N)
  } else {
    results$Normal    <- sample_size_normal_rel(mu, sigma, epsilon, conf,N)
    results$t         <- sample_size_t_rel(y, epsilon, conf, N)
    results$Gamma     <- sample_size_Gamma_rel(alpha_gamma, beta_gamma, epsilon, conf,N)
    results$Lognormal <- sample_size_Lognorm_rel(meanlog, sdlog, epsilon, conf,N)
  }

  # --- Formatted output ---
  cat("Sample size results (Discrete =", Discrete, ", Relative precision =", epsilon, ")\n")
...
relative_sample_zip <- function(y, epsilon_rel, conf = 0.95, N = NULL) {
  ...
  n5 <- sample_size_t_rel(y, epsilon_rel, conf, N)

  cat("Sample Size Results (Relative Error =", epsilon_rel, ")\n")
  cat("Normal Distribution Sample Size:"); print(n3)
  cat("Student-t Distribution Sample Size:"); print(n5)
  cat("Poisson Distribution Sample Size:"); print(n2)
  cat("Negative Binomial Sample Size:"); print(n1)
  cat("ZIP Distribution Sample Size:"); print(n4)
...
absolute_sample <- function(y, epsilon, conf = 0.95, Discrete = TRUE, N = NULL) {
  ...
  # Ajuste Lognormal (corrigindo valores não positivos)
  y_pos <- y
  if (any(y <= 0)) {
    shift <- abs(min(y)) + 1e-6
    y_pos <- y + shift
  ...
  # Funções internas com correção de população finita
  ...
  # Saída formatada
  cat("Sample Size Results (Absolute Error =", epsilon, ", Discrete =", Discrete, ")\n")
  for (nm in names(results)) {
    cat("Distribution", nm, ":"); print(results[[nm]])
  }

  invisible(results)
}
# -------------------------------------------------------
# Function 6: Bootstrap-based Relative Precision (CORRECTED)
# -------------------------------------------------------
...
#' @param epsilon Numeric. Desired relative precision threshold (0.2).
...
#' @examples
#' y <- rpois(30, 5)
#' out <- bootstrap_precision(y, epsilon = 0.2, n_boot = 200)
#' out <- bootstrap_precision(y, epsilon = 0.2, n_boot = 200, N_pop=100)
#' @export
bootstrap_precision <- function(y, epsilon, n_boot = 1000, conf = 0.95, seed = 123, N_pop = NULL) {
  library(ggplot2)
  library(gridExtra)

  set.seed(seed)
  y <- na.omit(y)
  N <- length(y)  # sample size from data
  alpha <- 1 - conf
  ...
  idx <- which(results$precision < epsilon)
  ...
  # Finite population correction - Return both n0 and n_adj
  if (!is.null(N_pop)) {
  ...
  # Plot 1: Mean ± CI
  p1 <- ggplot(results, aes(x = n, y = mean)) +
    ...
    labs(
      title = "Bootstrap Mean and Confidence Intervals",
      x = "Sample Size (n)",
      y = "Mean ± CI"
    ) +
    annotate("text", x = n0, y = min(results$mean),
             label = paste("n0 =", n0), vjust = -1, color = "green") +
    theme_minimal()

  # Plot 2: Relative Precision
  p2 <- ggplot(results, aes(x = n, y = precision)) +
    geom_line(color = "red") +
    geom_hline(yintercept = epsilon, linetype = "dashed", color = "blue") +
    geom_vline(xintercept = n0, linetype = "dashed", color = "green") +
    labs(
      title = "Relative Precision vs Sample Size",
      x = "Sample Size (n)",
      y = "Relative Precision"
    ) +
    annotate("text", x = n0, y = max(results$precision),
             label = paste("n0 =", n0), vjust = -1, color = "green") +
    theme_minimal()

  # Add annotation for finite population if applicable
  if (!is.null(N_pop)) {
    p1 <- p1 +
      geom_vline(xintercept = n_adj, linetype = "dashed", color = "purple") +
      annotate("text", x = n_adj, y = max(results$mean),
               label = paste("n_adj =", n_adj), vjust = -1, color = "purple")

    p2 <- p2 +
      geom_vline(xintercept = n_adj, linetype = "dashed", color = "purple") +
      annotate("text", x = n_adj, y = max(results$precision) * 0.9,
               label = paste("n_adj =", n_adj), vjust = -1, color = "purple")
  }
  ...
  cat("Relative Precision Target:", epsilon, "\n")
  cat("Achieved Relative Precision:", round(ci_info$precision, 4), "\n")
  cat("Confidence Level:", conf, "\n")
  cat("Bootstrap Replicates:", n_boot, "\n")
  cat("Estimated Mean:", round(ci_info$mean, 4), "\n")
  cat("Confidence Interval: [",
      round(ci_info$lower, 4), ", ",
      round(ci_info$upper, 4), "]\n", sep = "")
  cat("Confidence Interval Width:", round(ci_info$ci_width, 4), "\n")
  cat("Pilot Sample Size:", N, "\n")
  cat("===============================================\n")

  # Return both n0 and n_adj like in Function 4
  results_list <- list(
    sample_sizes = sample_sizes,
    infinite_population = list(
      n0 = n0,
      mean = ci_info$mean,
      lower_CI = ci_info$lower,
      upper_CI = ci_info$upper,
      ci_width = ci_info$ci_width,
      relative_precision = ci_info$precision
    ),
    finite_population = if (!is.null(N_pop)) {
      list(
        n_adj = n_adj,
...
# -------------------------------------------------------
# Function 8: Sample Size for Binomial Proportion (Adapted)
# -------------------------------------------------------
...
#' @param E Numeric. Desired precision (margin of error, 0 < E < 1).
...
#' sample_size_binomial(x = y, E = 0.05)
#'
#' # Example with relative precision (5% of the proportion)
#' sample_size_binomial(p = 0.4, E = 0.05, conf = 0.95, N = 500, relative = TRUE)
#' @export
sample_size_binomial <- function(x = NULL, p = NULL, E, conf = 0.95, N = NULL, relative = FALSE) {

  # Validate inputs
  if (missing(E) || E <= 0) stop("Error 'E' must be > 0")
...
#' @param E Numeric. Desired absolute precision (margin of error, >0).
#' @param conf Numeric. Confidence level (default = 0.95).
#' @param N Optional. Population size for finite population correction (default = NULL).
#' @param relative Logical. If TRUE, interprets E as relative error (default = FALSE).
#'
...
#' sample_size_beta(x = x, E = 0.05)
#'
#' # Example with provided parameters
#' sample_size_beta(alpha = 2, beta = 5, E = 0.05, conf = 0.95, N = 500)
#' @export
sample_size_beta <- function(x = NULL, alpha = NULL, beta = NULL,
                             E, conf = 0.95, N = NULL, relative = FALSE) {

  if (missing(E) || E <= 0) stop("'E' must be > 0")
  if (conf <= 0 || conf >= 1) stop("'conf' must be in (0,1)")
  ...
  if (is.null(alpha) || is.null(beta)) {
    ...
    # Transformação Smithson & Verkuilen (2006) para 0/1
    if (any(x == 0 | x == 1)) {
      x <- (x * (n - 1) + 0.5) / n
    }
  ...
  # Interpretar E como absoluto ou relativo
  E_abs <- if (relative) E * mu else E
