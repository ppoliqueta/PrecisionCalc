---
title: "PrecisionCalc: Sample Size and Distribution Analysis"
author: "Paulo Paiva"
date: "2025-09-28"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{PrecisionCalc: Sample Size and Distribution Analysis}
...
The **`PrecisionCalc`** package provides tools to compute required sample sizes for various distributions, evaluate absolute or relative precision, and visualize observed data with fitted distributions.
...
`relative_sample()` computes the sample size required to achieve a relative error (epsilon) for discrete or continuous distributions.

```{r example_relative_sample, echo=TRUE, warning=FALSE, message=FALSE}
# Discrete data
relative_sample(y_counts, epsilon = 0.1, Discrete = TRUE)

# Continuous data
relative_sample(y_cont, epsilon = 0.1, Discrete = FALSE)

# Continuos data: ample size of a finite population (N_pop=300)
relative_sample(y_cont, epsilon = 0.1, Discrete = FALSE, N=300)
```
...
set.seed(123)
out <- bootstrap_precision(y_counts, epsilon = 0.2, n_boot = 200)
# Sample size achieving desired precision with CI
out
#Sample size of a finite population (N_pop=300)
out2 <- bootstrap_precision(y_counts, epsilon = 0.2, n_boot = 200, N_pop=300)
out2
```
...
sample_size_binomial (y_bin, p = NULL, E=0.02, conf = 0.95, N = NULL)
```
...
sample_size_beta(x = y_beta, E = 0.05, conf = 0.95)
...
These are important guidelines for effectively using the **`PrecisionCalc`** package functions:
...
3. **Bootstrap-based precision**  
   ...
   - This function estimates precision non-parametrically using resampling, and helps visualize how precision improves with increasing sample size. It works both for infinite or finite populations. If working with finite populations the population size (N_pop) should be provided.
